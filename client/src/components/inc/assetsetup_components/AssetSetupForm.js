...
class AssetSetupForm extends React.Component {
    ...
    componentDidUpdate(prevProps){
        if(prevProps.data.asset_tags_groupped!=this.props.data.asset_tags_groupped){
                this.setState({ list : this.props.data.asset_tags_groupped })
        }
        if(prevProps.data.asset_tags_category_groupped!=this.props.data.asset_tags_category_groupped){
                this.setState({ category_list : this.props.data.asset_tags_category_groupped })
        }
        if(prevProps.data.asset_tags_sub_category_groupped!=this.props.data.asset_tags_sub_category_groupped){
            this.setState({ sub_category_list : this.props.data.asset_tags_sub_category_groupped })
        }
    }
        ...
        if(origin=="1-1"){
            ...
            for(var c=0;c<this.state.list.length;c++){
                if(this.state.list[c]._id==value){
                    ...
                    this.setState({ AssetDescriptionCode : code, desc_code_disable : true },()=>{
                    })
                    break;
                }
            }
            ...
            for(var c=0;c<this.state.sub_category_list.length;c++){
                ...
                if(this.state.sub_category_list[c]._id==value){
                    ...
                    this.setState({ AssetSubCategoryCode : code, sub_cat_code_disable : true });
                }
            }
            if(code==""){
    ...
    render (){
        const description_options=this.state.list.map((data, index)=>{
            return [
                <Option key={index} value={data._id}>{data._id}</Option>
            ]
        })
        const description_code_options=this.state.list.map((data, index)=>{
            return [
                <Option key={index} value={data.data[0].asset_setup_ad}>{data.data[0].asset_setup_ad}</Option>
            ]
        })
        ...
        const category_code_option=this.state.category_list.map((data, index)=>{
            if(data.data[0].asset_setup_description==this.state.AssetDescription){
                return [
                    <Option key={index} value={data.data[0].asset_setup_ac}>{data.data[0].asset_setup_ac}</Option>
                ]
            }
        })
        const sub_category_option=this.state.sub_category_list.map((data, index)=>{
            if(data.data[0].asset_setup_description==this.state.AssetDescription && data.data[0].asset_setup_category==this.state.AssetCategory && data.data[0].asset_setup_category!=null && data.data[0].asset_setup_category!=""){
                return [
                    <Option key={index} value={data._id}>{data._id}</Option>
                ]
            }
        })
        const sub_category_code_option=this.state.sub_category_list.map((data, index)=>{
            if(data.data[0].asset_setup_description==this.state.AssetDescription && data.data[0].asset_setup_category==this.state.AssetCategory && data.data[0].asset_setup_category!=null && data.data[0].asset_setup_category!=""){
                return [
                    <Option key={index} value={data.data[0].asset_setup_sc}>{data.data[0].asset_setup_sc}</Option>
                ]
            }
        })
        return [
            <div>
                <Row gutter="16" align={'middle'}>
                    ...
                    <Col span={6} >
                        <AutoComplete
                            ...
                            onChange={(event)=>this.setState({ AssetDescription : event },()=>{this.set_other_value('1-1','1-2',event)})}
                            filterOption={true}
                            style={{width : '100%'}}
                    ...
                    <Col span={6} >
                        ...
                        disabled={this.state.desc_code_disable}
                            onChange={(event)=>this.setState({ AssetDescriptionCode : event },()=>{this.set_other_value('1-2','1-1',event)})}
                            filterOption={true}
                ...
                <Row gutter="16" align={'middle'} style={{marginTop : '10px'}}>
                    ...
                    <Col span={6} >
                        <AutoComplete
                            className="AutoCompleteCapitalize"
                            onChange={(event)=>this.setState({ AssetCategory : event },()=>{this.set_other_value('2-1','2-2',event)})}
                            filterOption={true}
                            style={{width : '100%'}}
                    ...
                    <Col span={6} >
                        <AutoComplete
                            ...
                            onChange={(event)=>this.setState({ AssetCategoryCode : event },()=>{this.set_other_value('2-2','2-1',event)})}
                            filterOption={true}
                            style={{width : '100%',textTransform : 'uppercase'}}
                ...
                <Row gutter="16" align={'middle'} style={{marginTop : '10px'}}>
                    ...
                    <Col span={6} >
                        <AutoComplete
                            className="AutoCompleteCapitalize"
                            onChange={(event)=>this.setState({ AssetSubCategory : event },()=>{this.set_other_value('3-1','3-2',event)})}
                            filterOption={true}
                            style={{width : '100%'}}
                    ...
                    <Col span={6} >
                        <AutoComplete
                            ...
                            className="AutoCompleteUpperCase"
                            onChange={(event)=>this.setState({ AssetSubCategoryCode : event },()=>{this.set_other_value('3-2','3-1',event)})}
                            filterOption={true}
                            style={{width : '100%',textTransform : 'uppercase'}}
                ...
                <Row gutter="16" align={'middle'} style={{marginTop : '10px'}}>
                    ...
                    <div>
                    <Button type="primary" style={{marginRight : '10px'}} onClick={()=>this.submit_asset_setup()}>Save</Button>
