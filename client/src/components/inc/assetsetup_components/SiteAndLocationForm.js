...
class SiteAndLocationFrom extends React.Component {
        ...
        if(this.state.location!=""){
            if(this.state.site!=""){
                ...
                for(var c=0;c<this.props.asset_site_and_location;c++){
                    ...
                    if(dat.asset_setup_location==this.state.location && dat.asset_setup_site==this.state.site){
                        duplicate=1;
                        break;
                    }
                }
                if(duplicate==0){
                    ...
                    const response = await axios.post(api_base_url+'/api/save_site_and_location',
                    {
    ...
    componentDidUpdate(prevProps){
        if(prevProps.data.asset_site_groupped!=this.props.data.asset_site_groupped){
                this.setState({ sites_groupped : this.props.data.asset_site_groupped })
        }
        if(prevProps.data.asset_location_groupped!=this.props.data.asset_location_groupped){
                this.setState({ location_groupped : this.props.data.asset_location_groupped })
        }
    }
    componentDidMount(){
        this.setState({ sites_groupped : this.props.data.asset_site_groupped })
    ...
    render (){
        ...
        const site_list= this.state.sites_groupped.map((data, index)=>{
            if(data.data[0].asset_setup_location==this.state.location && data.data[0].asset_setup_site!='' && data.data[0].asset_setup_site!=null && data.data[0].asset_setup_site!=undefined){
                return [
                    <Option value={data.data[0].asset_setup_site}>{data.data[0].asset_setup_site}</Option>
                ]
            }
        ...
        return [
            <div>
                ...
                <Row gutter="16" align={'middle'} style={{marginTop : '10px'}}>
                    ...
                    <div>
                    <Button type="primary" style={{marginRight : '10px'}} onClick={()=>this.submit_site_and_location()}>Save</Button>
